...
- Recent achievements/milestones (Future: Phase 4+)
...
├── TemplateLibrary.tsx       // Pre-built scenarios (Future: Phase 4+)
...
- Template library (SaaS, Real Estate, Insurance, etc.) (Future: Phase 4+)
...
> **Note:** The actual implemented schema is in `/docs/DATABASE_SETUP.sql`. This section reflects the current implementation.
...
-- 2. PRODUCTS (reusable product profiles)
...
-- 3. SESSIONS (practice session records)
CREATE TABLE sessions (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id UUID NOT NULL REFERENCES profiles(id) ON DELETE CASCADE,
    product_id UUID REFERENCES products(id) ON DELETE SET NULL,

    -- Scenario data (generated by AI)
    ...
    overall_grade TEXT,  -- A+, A, A-, B+, etc.

    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- 4. TRANSCRIPTS (conversation transcripts)
...
-- 5. ANALYSES (performance analysis results)
...
CREATE VIEW user_skill_breakdown AS
SELECT
    s.user_id,
...
All tables have RLS enabled with policies ensuring users can only access their own data. See `/docs/DATABASE_SETUP.sql` for complete policy definitions.

### Tables Not Yet Implemented (Future Enhancements)

The following tables from the original design are **not** in the current implementation but may be added later:

```sql
-- TEMPLATES (pre-built scenarios) - Phase 4+
-- ACHIEVEMENTS (gamification) - Phase 4+
-- AUDIO_FILES (audio storage metadata) - Currently using Supabase Storage directly
```
...
# User Profile
GET    /api/users/me  # or /api/profiles/me
PATCH  /api/users/me
...
# Templates (Not Implemented - Phase 4+)
# GET    /api/templates?category=saas&difficulty=medium
# GET    /api/templates/:id

# Achievements (Not Implemented - Phase 4+)
